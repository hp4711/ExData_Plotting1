...
par(mfrow = c(2, 2))
with(pc, {
    plot(pc$DateTime, pc$Global_active_power, type = "l", xlab = "", ylab = "Global Active Power")

    plot(pc$DateTime, pc$Voltage, type = "l", xlab = "datetime", ylab = "Voltage")
    
    plot(pc$DateTime, pc$Sub_metering_1, type = "l", col="black", xlab = "", ylab = "Energy sub metering", ylim = c(0, 38))
    par(new = T)
    plot(pc$DateTime, pc$Sub_metering_2, type = "l", col="red", xlab = "", ylab = "Energy sub metering", ylim = c(0, 38))
    par(new = T)
    plot(pc$DateTime, pc$Sub_metering_3, type = "l", col="blue", xlab = "", ylab = "Energy sub metering", ylim = c(0, 38))
    par(new = F)
